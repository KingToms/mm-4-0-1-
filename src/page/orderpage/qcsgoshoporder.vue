<template>
  <div class="order" :class="{'bgColor': page === 'coupon'}">
    ...
    <div class="main appoint-shop" v-if="this.page==='serviceshop'">
        <serviceshop :sendproid="product_info.product_id" @closeAppoint="chooseAppoint('order','确认订单')"></serviceshop>
    ...
    <div class="main order-info" v-if="this.page === 'order'">
      ...
      <section class="product bgwhite mgt">
        <div class="shop-info">
          ...
          <!-- <span>{{stylists.length>0?stylists[0].store_name:confirmOrder.store_name}}</span> -->
          <span>{{confirmOrder.store_name }}</span>
          <div class="star">
...
export default { 
    ...
    computed: {
        ...
        // newstore_id(){
        //     return this.confirmOrder.store_id
        // }
    },
    watch:{
        stylist_id(val,oldval){
            this.changeTimeData()
        }
    ...
    methods: {
        ...
        async getstoreInfo(){
            let obj={
                storeId:this.confirmOrder.store_id
            }
        ...
        chooseAppoint(page, title) {
            ...
            return
            this.stylist_id=_this.confirmOrder.store_id
            this.page = page;
            if(page=="time"&&_this.confirmOrder.store_name!==''){
                _this.stylist_id=_this.confirmOrder.store_id
            }
            this.title = title;
        ...
        async getData() {
            let _this=this
            let id = _this.product_info.product_id
        ...
        async payment() {
            ...
            if(this.inDoor == 2 && !this.confirmOrder.store_name){
                alert('请填写预约门店')
                return
            }
            ...
            if(this.inDoor == 1 && !this.confirmOrder.address){
                alert('请填写预约地址')
                return
            }
        console.log(this.stylist_id)
            ...
            let address,storeId;
            if(this.inDoor==1){
                storeId=_this.confirmOrder.store_id
                address=_this.confirmOrder.address
                console.log(this.stylist_id,address)
            }else{
                storeId=_this.confirmOrder.store_id
                address=_this.confirmOrder.store_address
                console.log(storeId,address)
            }
            if(this.product_info.product_id==1000069){
                if(_this.confirmOrder.sn==0){
                    console.log(1)
                }else{
                    console.log(2)
                }
