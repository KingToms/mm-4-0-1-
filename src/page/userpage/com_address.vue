    ...
    <section class="select-address" v-if="!newAddState">
      <ul class="address-info">
        <li class="addItem" v-for="(item,index) in filterAddress" :key="index">
          <span class="detail" :class="{'inconsistent': false}">
...
import {getStore} from '../../../src/common/store.js'; // 获取store中 当前城市
...
export default {
  ...
  methods: {
    ...
    /* 返回上一级 */
    back (){
      if(this.title === '常用地址修改'){
        this.newAddState = false; // 隐藏修改页面
    ...
    currentAddress (index,item){
      ...
      this.confirmOrder.selected = index;  // 索引
      this.confirmOrder.province_id = item.province ; // 省ID
      this.confirmOrder.city_id = item.city; // 城市ID
      this.confirmOrder.district_id = item.district; // 区ID
      this.confirmOrder.street_address = item.address; // 详细街道
      this.confirmOrder.address = item.province_name + item.city_name + item.district_name + item.street_name + item.address; // 地址项
