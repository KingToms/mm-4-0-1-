<template>
  <div class="address-module">
    <div v-show="!chooseCity">
      ...
      <section class="new-address" v-if="newAddState">
        <div class="current-city">
          <div class="city" :class="{'color-tip': cityName}" @click="changeCity">
            <i class="icon"></i>
            {{cityName}}
          </div>
        </div>
        <div class="detail">
          <ul class="detail-add">
            <li @click="areaChoose">
              <span>所在地区</span>
              <div class="more" :class="{'color-black': Localarea}" href="javascript:void(0);">{{Localarea ? Localarea : "请选择"}}</div>
              <i class="icon"></i>
            </li>
            <li>
              <span>详细街道</span>
              <textarea :class="[address_Tip=='街道\街道号等' ? '' : 'color-black' ]" v-model="address_Tip" class="more" rows="2" @focus="clearContent" @blur="noChange"></textarea>
            </li>
          </ul>
        ...
        <div class="trading-area">
          ...
          <div class="district">
            <div class="district-detail" v-if="!tradingArea">
              <ul class="detail">
                <li v-for="(item1, index) in region" :key="index">
                  <span :class="{'select': index == selectArea}" @click="currentArea(index,item1)">{{item1.name}}</span>
                  <i class="icon" v-if="index == selectArea"></i>
                </li>
              </ul>
            </div>
            <div class="district-sort district-detail scroll" v-if="tradingArea">
              <ul class="detail">
                <!-- <li v-for="(item2, index) in tradeArea" :key="index">
                    <span @click="currentTrade(item2)">{{item2.name}}</span>
                  </li> -->
                <li v-for="(item2, index) in tradeArea" :key="index">
                  <span :class="{'select': index == selectTrading}" @click="currentTrade(index,item2)">{{item2.name}}</span>
                  <i class="icon" v-if="index == selectTrading"></i>
                </li>
              </ul>
            </div>
          </div>
    ...
    <city v-show="chooseCity" @hiddenCity="hiddenCity"></city>
  </div>
</template>
<script>
...
import { getAddress, saveAddress, delateAddress, areaList, streetList } from '@/service/getData'; // 获取用户地址列表
...
export default {
  ...
  methods: {
    ...
    async getAddressList() {
      ...
      this.$emit('showAddressEdit', this.filterAddress.length);
    },
    /*保存用户服务地址*/
    ...
    newAddress(index) {
      if (this.bottomText == "添加新地址") {
        ...
        // if (!this.Localarea || !this.trading) {
        if (!this.Localarea) {
          alert("请选择正确的服务地区");
    ...
    hiddenCity() {
      ...
      this.Localarea = '';
      this.cityStorage = JSON.parse(getStore("current_city"));
      this.cityName = this.cityStorage.city_name;
      this.getCityStreet(this.cityStorage.city_id);
    }
  }
}
</script>
    ...
    .current-city {
      ...
      .city {
        ...
        i.icon {
          ...
          @include bgi_2('/static/icon/order/address_icon_position.png', 2rem, 2rem, center, center, no-repeat);
        }
        &.color-tip {
          color: #999;
        }
      ...
      .detail-add {
        ...
        li {
          ...
          i.icon {
            ...
            @include bgi_2('/static/icon/order/icon_next.png', .7rem, 1.3rem, center, center, no-repeat);
          }
        }
        li:nth-of-type(1) {
          @include average(#ccc);
    ...
    .address-info {
      ...
      li.addItem {
        ...
        i.icon {
          ...
          right: 2rem;
          margin-top: -1rem;
          @include bgi_2('/static/icon/order/btn_choose_nor.png', 2rem, 2rem, center, center, no-repeat);
        }
        i.nowAddress {
          @include bgi_2('/static/icon/order/btn_choose_sel.png', 2rem, 2rem, center, center, no-repeat);
        }
        span.inconsistent {
          color: #999;
        }
        ...
        .edit-box {
          ...
          li.left:nth-of-type(2) {
            margin-top: 0.5rem; // width: 0.2rem;
            height: 1.6rem;
            margin: 0 1rem;
            div.vertical {
              position: absolute;
  ...
  .bottom-btn {
    ...
    .btn {
      ...
      i.icon {
        ...
        left: 50%;
        margin-left: -21%;
        @include bgi_2('/static/icon/order/address_icon_add.png', 1.9rem, 1.9rem, center, center, no-repeat);
      }
    }
  }
  /*商圈选择*/
          ...
          ul.detail {
            ...
            li {
              ...
              i.icon {
                ...
                @include bgi_2('/static/icon/order/address_icon_choose.png', 1.7rem, 1.7rem, center, center, no-repeat);
              }
            }
          }
        }
