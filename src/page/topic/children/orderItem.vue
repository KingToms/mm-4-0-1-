      ...
      <div class="item" v-for="(item,index) in orderList" :key="index" @click="$router.push({name: 'orderDetail', params: {ordersn: item.sn}})">
        ...
        <div class="product-info">
          <div class="info">
            ...
            <p class="price"><span>￥ {{(item.products[0]).price}} <del>￥ {{(item.products[0]).market_price}}</del></span><span class="sum"><span>X </span> {{item.products.length}}</span></p>
          </div>
...
import {startService, endService, cancelOrder} from '../../../service/getData'
export default {
  name: 'orderItem',
  data () {
    return {
