<template>
  <div class="city">
    ...
    <div class="content" v-show="unSearch">
      <div class="content-left">
        <section class="currentCity">
          ...
          <div class="city"@click="hiddenCity(currentCity)">{{currentCity.name}}</div>
        </section>
        <section class="popularCity">
          <div class="title_city">热门城市</div>
          <ul class="cityDetail clear">
            <li class="city" v-for="(item,index) in popularCity" :key="index" @click="hiddenCity({name: item, value: sourceData[item]})"><a href="javascript:void(0);">{{item}}</a></li>
          </ul>
        </section>
        <section class="openCity">
          ...
          <div v-for="item in arrTop" :key="item" :id="item">
            <div v-if="cityArr[item]">
              ...
              <div v-for="city in cityArr[item]" :key="city" class="city_item" @click="hiddenCity(city)">{{city.name}}</div>
            </div>
          </div>
        </section>
      </div>
    ...
    <div class="searchResult" v-show="!unSearch">
      <div v-for="city in searchResult" :key="city" class="city_item" @click="hiddenCity(city)">{{city.name}}</div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    showPosition (position){
      let storeCity = getStore('current_city');
      let cityObj = storeCity && storeCity.length >0 ? JSON.parse(storeCity) : {"city_name":"广州市","city_id": "440100"};
      cityObj.city_name = position.city ? position.city : cityObj.city_name;
      cityObj.city_id = position.adcode ? position.adcode : cityObj.city_id;
      cityObj.city_id = cityObj.city_id.toString().substring(0,4) + "00"; // 确保获取的是城市ID，而不是区ID.
      // let cityObj = 
      localStorage.setItem("current_city", JSON.stringify(cityObj)); // localStore存储当前城市及ID

      this.currentCity = {"name": cityObj.city_name,"value": {"city_id": cityObj.city_id}};
      this.nowCity = cityObj.city_name; // 显示当前城市
    },
    showErr (){
      let cityObj = getStore('current_city');
      let storeCity = cityObj && cityObj.length > 0 ? JSON.parse(cityObj) : {"city_name": "广州市","city_id": "440100"};
      localStorage.setItem("current_city", JSON.stringify(storeCity));
      alert("定位失败！请刷新");
    },
    setCity(){
      let cityObj = getStore('current_city');
      let storeCity = cityObj && cityObj.length > 0 ? JSON.parse(cityObj) : {"city_name": "广州市","city_id": "440100"};
      this.currentCity = {name: storeCity.city_name, value: {city_id: storeCity.city_id} };
    },
    async getCities(){
    ...
    hiddenCity(city){
      ...
      this.currentCity = {name: city.name, value: city.value.city_id};
      setStore("current_city",obj);
      this.$emit("hiddenCity");
    }
  }
}
</script>
